...
document.addEventListener("DOMContentLoaded", function() {
  ...
  submit.addEventListener('click', function(event){
      ...
      if (selectedPlantObject) {
          ...
          let recentlySearchedPlants = JSON.parse(localStorage.getItem('recentlySearchedPlants')) || [];
          recentlySearchedPlants.push(selectedPlantObject);
              if (recentlySearchedPlants.length > 3) {
                  recentlySearchedPlants = recentlySearchedPlants.slice(recentlySearchedPlants.length - 1);
              }
          localStorage.setItem('recentlySearchedPlants', JSON.stringify(recentlySearchedPlants));
          const recentlySearched = document.getElementById('recentlySearched');
          recentlySearched.innerHTML = '';
          recentlySearchedPlants.forEach((plant, index) => {
              recentlySearched.innerHTML += `
                  <div>
                      <li>Plant Name: ${plant.name}</li>
                      <li>Ideal Planting Month: ${plant.IdealPlantingMonth}</li>
                      <li>Days to Maturity: ${plant.DaytoMaturity}</li>
                      <li>Annual or Perennial: ${plant.AnnualPerrenial}</li>
